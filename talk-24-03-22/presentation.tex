...
\begin{itemize}
	...
	\item OTF heuristic overall best approach finding for almost all instances: 
		...
		\end{itemize}
	\item [$\rightarrow$] caching of fragments' results and carefully crafted evaluation with OTF charging station insertion pays off!
\end{itemize}
}
...
\begin{itemize}
	...
	\item[$\rightarrow$] e.g., apply LNS for static problem
...
\begin{frame}{Mean Order Delivery Time / Driver Performance}% \\ \small \textcolor{gray}{Beardwood et al. (1959), Daganzo (1984), Figliozzi (2008, 2010)}}
	Given a day $a$ and an hour $t$, we define the \alert{mean order delivery time} $\phi_t(a)$, the time needed by an average driver to deliver one order.
	\bigskip
	
	%\textbf{Aim of this work}. Find a good predictor $\tilde \phi_t(a)$ for $\phi_t(a)$.
	%\structure{Bootstrapping}: Consider active labor time, subsequent shift planning will result into tight shifts with almost no waiting times.
	%\bigskip
	
	%Is necessary for:
	%\begin{itemize}
	%	\item Shift planning
	%	\item Large horizon planning
	%	\item Waiting time strategies
	%\end{itemize}
...
\begin{frame}
	\frametitle{Learning Surrogate Models\\ \small \textcolor{gray}{Bracher, Frohner, and Raidl (2021)}}
	...
	\begin{itemize}
		\item replace route duration $d(r)$ by parameterized \alert{surrogate travel time} $d'(r)$
		\item predicts the \alert{future} travel time of delayable orders/routes
		\item supervised learning in an offline phase using scenario sampling
		\item resulting surrogate directly used in online LNS optimization
	\end{itemize}
	\bigskip
	
	\pause
	\textbf{Key results:} up to 5--10\% improvement in travel times with 30-40 orders per hour and sufficient drivers to avoid tardiness.
\end{frame}

\begin{frame}
	\frametitle{Results}
	\centering
	\includegraphics[width=0.4\textwidth]{graphics/boxplots.pdf}
\end{frame}
...
\begin{frame}[allowframebreaks]
	...
	\bibliography{eurocast-2024-presentation_sources}
\end{frame}
