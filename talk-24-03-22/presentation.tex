...
\structure{Greedy heuristic:} {\footnotesize{\textcolor{gray}{(\cite{Ropke:2006})}}}
\begin{itemize}
...
\begin{frame}
  ...
  \framesubtitle{Time-Efficient Heuristic I}

\only<1>{
\structure{Workflow:}
\begin{itemize}
	\item Remove charging stops from route 
	\item Determine remaining route~$R$: fragments and depots 
	\item Forward pass: % determine  
		\begin{itemize}
...
Cordeau instances with $\gamma = 0.1$
...
Cordeau instances with $\gamma = 0.7$ 
...
Ropke instances with $\gamma = 0.1$
...
Ropke instances with $\gamma = 0.7$
...
\begin{itemize}
	...
	\item OTF heuristic better than OTF MILP approach 
		\begin{itemize}
...
%	\item Consideration of very large-scale instances with different LNS operators 
	\item Very large-scale instances with different LNS operators 
	\item Machine learning-based operator selection 
