...
\begin{frame}{Generative Flow Networks (GFlowNets)}
	...
	\medskip
	\begin{itemize}
		\item \important{stochastic policy} (generative model) approximating a \textbf{\important{probability distribution}},
	...
	\item related to RL, but RL trained to find just \alert{one ``best'' solution}!
	\end{itemize}
\end{frame}

\begin{frame}{GFlowNet Training Losses}
%\small
...
%\end{block}
\medskip
\structure{Goal:} \important{Achieve flow consistency}, \alert{not maximze reward} as in RL!
...
\begin{frame}{Advantages of GFlowNets}
	...
	\begin{itemize}
		...
		\item \important{Do not get trapped so easily in objects reachable by many trajectories}
...
\begin{frame}{GFlowNets for Selecting Destroy Sets in Large Neighborhood Search}
	\structure{\it Ongoing work with M.~Bresich, J.~Peng, L.~Tomandl}
	
	\medskip
	\begin{itemize}
		\itemsep0.5ex
		\item \structure{Large neighborhood search:} iteratively \important{destroy} parts of a solution and \important{repair} %,\\ e.g., by an ILP or iterated greedy
		\item GFlowNet trained to \important{select promising parts to destroy}
		\pause
		\item benchmarked on MISP, MaxCut, Densest $k$-Subgraph problems\\[2.5ex]
		\begin{center}
		\includegraphics[width=0.57\textwidth]{graphics/gflownet4lns.png}
		\end{center}
		\item GFlowNets \alert{particularly well suited} as they produce \important{\textbf{different}} promising destroy sets!
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\itemsep2.5ex
		\item \structure{Can today's LLMs be used to solve COPs?}
		\begin{itemize}
