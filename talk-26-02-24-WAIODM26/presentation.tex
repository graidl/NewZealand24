...
\begin{frame}{GFlowNets for Selecting Destroy Sets in Large Neighborhood Search}
	...
	\begin{itemize}
		\itemsep1ex
		\item \structure{Large neighborhood search:} iteratively \important{destroy} parts of a solution and \important{repair} them,\\ e.g., by an ILP or iterated greedy
		\item GFlowNet trained to \important{select promising parts to destroy}
		\item benchmarked on MISP, MaxCut, densest $k$-subgraph problems
		\item GFlowNets are \alert{doing here particularly well}!
		\begin{itemize}
			\item able to produce \important{many different promising destroy sets}
			\item other ML-methods tend to collapse into uni-modal trap
		\end{itemize}
	\end{itemize}

	\medskip
	\begin{center}
		\includegraphics[width=0.8\textwidth]{graphics/gflownet4lns.png}
	\end{center}
...
\begin{frame}{Denoising Diffusion Models (DDMs)}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Neural network trained to remove noise from intermediate noisy images
		\end{itemize}
...
\begin{frame}
	...
	\medskip
	\begin{center}
	\includegraphics[width=0.7\textwidth]{graphics/ddbea.png}
	\end{center}
\end{frame}
